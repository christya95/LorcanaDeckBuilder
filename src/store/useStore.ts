...
interface State {
  cards: Card[];
  index: any; // Changed from elasticlunr.Index<Card> | null due to TS namespace issue
  indexReady: boolean;
  loading: boolean;
  decks: DeckMeta[];
...
export const useStore = create<State>((set, get) => ({
  ...
  search: (q: string) => {
    ...
    return index.search(q, { expand: true }).map((r: any) => index.documentStore.getDoc(r.ref));
  },
